...
Be aware of the environment in which the ASP.NET Core development certificate is being installed. It's important to avoid having it installed in an environment that will be redistributed, such as a container image. Redistribution of the certificate's private key risks it being maliciously used to spoof the identity of a victim. In order to prevent the certificate from being generated during the .NET CLI's first-run experience, set the `DOTNET_GENERATE_ASPNET_CERTIFICATE` environment variable to `false`.
